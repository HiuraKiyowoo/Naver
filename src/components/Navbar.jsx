...
const Navbar = () => {
  ...
  useEffect(() => {
    const t = setTimeout(async () => {
      if (query.length > 2) {
        ...
        try { setLiveResults((await search(query)).items || []); } catch { setLiveResults([]); }
  ...
  const navLinks = [
    ...
    { path: '/browse',  label: 'Browse',  icon: <path strokeLinecap="round" strokeLinejoin="round" d="M4 6h16M4 10h16M4 14h16M4 18h7"/> },
  ...
  const notchPos = activeIndex >= 0 ? `${activeIndex * 25 + 12.5}%` : '12.5%';
  ...
  return (
    <>
      <style>{`
        ...
        .notch-nav::before{content:'';position:absolute;top:-1px;left:${notchPos};transform:translateX(-50%);width:58px;height:28px;background:rgba(10,10,12,.95);border-radius:0 0 50% 50%/0 0 100% 100%;border:1px solid rgba(255,255,255,.08);border-top:none;transition:left .5s cubic-bezier(.34,1.56,.64,1);z-index:0}
        ...
        .notch-icon.active{background:#F6CF80;transform:translateY(-14px) scale(1.1);box-shadow:0 8px 24px rgba(246,207,128,.35)}
        ...
        .csb::-webkit-scrollbar{width:4px} .csb::-webkit-scrollbar-thumb{background:rgba(255,255,255,.1);border-radius:4px}
      `}</style>
      ...
      <nav className="fixed top-2 inset-x-4 z-[100] max-w-7xl mx-auto">
        <div className="bg-black/60 h-16 px-6 rounded-2xl flex items-center justify-between border border-white/5 shadow-lg relative overflow-hidden">
          <div className="flex items-center gap-1 cursor-pointer z-10 shrink-0" onClick={() => navigate('/home')}>
            <span className="text-[#F6CF80] font-black text-lg tracking-tight">Naver</span>
        ...
        {isSearchOpen && query.length > 2 && (
          <div className="absolute top-20 left-4 right-4 md:w-96 bg-[#16161a] border border-white/10 rounded-2xl shadow-2xl z-[110] max-h-[60vh] overflow-y-auto csb origin-top animate-[slideDown_.2s_ease-out]">
            {isLiveLoading ? (
              <div className="p-6 text-center text-[#F6CF80] text-xs font-bold">mencari...</div>
            ) : liveResults.length > 0 ? liveResults.map((r, i) => (
              <div key={r.slug || i} onClick={() => { navigate(`/novel/${r.slug}`); setIsSearchOpen(false); }} className="flex items-center gap-4 p-3 hover:bg-white/5 cursor-pointer border-b border-white/5 transition-colors">
                <div className="flex flex-col">
                  <span className="text-white font-bold text-xs line-clamp-1">{r.title}</span>
                  {r.type && <span className="text-white/40 text-[9px] font-bold mt-1">{r.type}</span>}
                </div>
              </div>
            )) : (
